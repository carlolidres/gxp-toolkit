...
function loadScheduler(byName, byCode) {
  ...
  for (let i = 4; i < rows.length; i += 1) {
    ...
    seq += 1
    const year = coverage.end.slice(0, 4)
...
function main() {
  ...
  const years = [...new Set(schedulerEntries.map((e) => Number(e.apqr_id.slice(5, 9))))]
  const sqlLines = [
  ...
  for (const year of years.sort()) {
    const count = schedulerEntries.filter((e) => e.apqr_id.startsWith(`APQR-${year}-`)).length
    sqlLines.push(`INSERT INTO apqr_id_sequences (year, last_number) VALUES (${year}, ${count});`)
  }
  ...
  for (const year of years.sort()) {
    const count = schedulerEntries.filter((e) => e.apqr_id.startsWith(`APQR-${year}-`)).length
    pgLines.push(`INSERT INTO public.apqr_id_sequences (year, last_number) VALUES (${year}, ${count});`)
  }

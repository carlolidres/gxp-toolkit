 ...
 * Mirrored in supabase/functions/_shared/edocCompletionHistory.ts for Edge Functions.
 */

import { isApprovedSignatureMeaning, isReviewedSignatureMeaning } from './signatureMeanings'
...
export function buildCompletionHistoryEvents(input: {
  ...
  for (const row of audits) {
    ...
    if (
      row.event_type === 'approve_completed'
      && signatures.some((sig) => isApprovedSignatureMeaning(sig.signature_meaning))
    ) {
      continue
    }
    ...
    if (row.reason) {
      details.push(
        row.event_type === 'signer_note' ? `Note: ${row.reason}` : `Reason: ${row.reason}`,
      )
    }
  ...
  for (const sig of signatures) {
    ...
    const isApprovedBy = isApprovedSignatureMeaning(sig.signature_meaning)
    const signerName = (sig.signer_display_name || '').trim() || 'Signer'
    candidates.push({
      id: sig.id,
      ...
      event: {
        title: isReviewedBy
          ...
          : isApprovedBy
            ? `Document approved and electronically signed by ${signerName}`
